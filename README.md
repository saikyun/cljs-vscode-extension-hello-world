...
3.1 Stop debugging
3.2 Write the following in a terminal (if in VSCode, preferably the one you starting debugging from):
```
...
3.3 Start debugging again
3.4 Run the "Hello World" command (this loads your code into vscode)
3.5 Try evaling something at the cljs-repl in the terminal again. Now it should work. :)
...
4.1 Assuming Calva, you can use the command "Calva: Connect", either in the debugging instance (for that emacs feeling), or in the vscode instance that started debugging.
4.2 It's important that you fill in the port you got from running `shadow-cljs clj-repl` before.
